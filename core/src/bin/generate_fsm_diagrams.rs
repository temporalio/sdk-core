...
fn main() {
    ...
    if !d2_available() {
        eprintln!("Error: d2 is not installed. SVG generation and index.html will not be produced.\nInstall d2: https://d2lang.com/tour/install");
        std::process::exit(1);
    }
    ...
    let _ = Command::new("cargo")
        ...
        .env(
            "TEMPORAL_FSM_DIAGRAM_DIR",
            diagram_dir.as_os_str(),
        )
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null())
    ...
    if count_ext(&diagram_dir, "d2") == 0 {
        eprintln!("Error: No .d2 diagrams were generated. Ensure state machines compile and try again.");
        std::process::exit(2);
    }
...
fn write_index_html(diagram_dir: &Path, _use_svg: bool) -> io::Result<()> {
    #[derive(Clone)]
    struct Item { stem: String, id: String, label: String }
        ...
        for entry in entries.flatten() {
            ...
            if p.extension().and_then(|e| e.to_str()) == Some("svg") {
                if let Some(stem) = p.file_stem().and_then(|s| s.to_str()) {
                    ...
                    let label = display_label_from_d2(diagram_dir, stem).unwrap_or_else(|| stem_s.clone());
                    let id = label.clone();
                    items.push(Item { stem: stem_s, id, label });
                }
            }
        }
    }
    ...
    for it in &items {
        html.push_str(&format!("<a href=\"#{}\"><code>{}</code></a>\n", it.id, it.label));
    }
    html.push_str("</nav>\n");
    ...
    for it in &items {
        ...
        html.push_str(&format!("<img class=\"diagram\" src=\"{}\" alt=\"{}\"/>\n", src, it.label));
...
fn display_label_from_d2(diagram_dir: &Path, stem: &str) -> Option<String> {
    ...
    for line in content.lines().take(5) {
        if let Some(name) = line.strip_prefix('#').and_then(|rest| rest.trim().strip_suffix(" State Machine")) {
            let mut s = to_snake(name.trim());
            if let Some(stripped) = s.strip_suffix("_machine") { s = stripped.to_string(); }
            return Some(s);
        }
    }
    None
...
fn to_snake(s: &str) -> String {
    ...
    for ch in s.chars() {
        if ch.is_ascii_uppercase() {
            if prev_lower && !out.is_empty() { out.push('_'); }
            out.push(ch.to_ascii_lowercase());
            prev_lower = false;
        } else {
            let is_sep = ch == ' ' || ch == '-' || ch == '/';
            if is_sep {
                if !out.ends_with('_') && !out.is_empty() { out.push('_'); }
                prev_lower = false;
            } else {
                out.push(ch);
...
fn count_ext(dir: &Path, ext: &str) -> usize {
    ...
    if let Ok(entries) = fs::read_dir(dir) {
        for e in entries.flatten() {
            let p = e.path();
            if p.extension().and_then(|e| e.to_str()) == Some(ext) { c += 1; }
        }
    }
    c
}
