mod heavy_tests {
    ...
    use futures::{future::join_all, sink, stream::FuturesUnordered, FutureExt, StreamExt};
    ...
    use temporal_sdk::{
        ActContext, ActivityOptions, LocalActivityOptions, WfContext, WorkflowResult,
    };
    ...
    async fn many_parallel_timers_longhist(ctx: WfContext) -> WorkflowResult<()> {
        for _ in 0..120 {
            let mut futs = vec![];
            for _ in 0..100 {
    ...
    async fn fuzzy_workflow() {
        ...
        let mut actions: Vec<FuzzyWfAction> =
            rng.sample_iter(FuzzyWfActionSampler).take(15).collect();
