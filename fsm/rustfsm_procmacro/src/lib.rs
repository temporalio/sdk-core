    ...
    #[cfg(not(doc))] // Don't generate diagrams during doc builds
    {
        if env::var("TEMPORAL_GENERATE_FSM_DIAGRAMS").is_ok() {
            ...
            if let Err(e) = fs::create_dir_all(&out_dir) {
                eprintln!(
                    "Warning: Failed to create diagram directory {}: {}",
                    out_dir, e
                );
            } else {
...
fn is_proto_history_event(name: &str) -> bool {
    ...
    for candidate in [
        candidate.clone(),
        candidate.replace("CANCELLED", "CANCELED"),
    ] {
        if temporal_sdk_core_protos::temporal::api::enums::v1::EventType::from_str_name(&candidate)
            .is_some()
        {
            return true;
        }
