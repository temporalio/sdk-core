...
impl WorkflowFuture {
    ...
    fn poll_signal_futures(&mut self, cx: &mut Context) -> Result<(), Error> {
        let result: Result<Vec<_>, _> = std::mem::take(&mut self.signal_futures)
            .into_iter()
        ...
        match result {
            Ok(remaining) => {
                self.signal_futures = remaining;
                Ok(())
            }
            Err(e) => Err(anyhow!("Signal handler error: {}", e)),
        }
    }

    /// Poll all in-progress update futures, removing completed ones.
