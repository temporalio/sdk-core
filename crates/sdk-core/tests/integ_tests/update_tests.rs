...
use temporalio_client::{
    Client, NamespacedClient, WorkflowClientTrait, WorkflowOptions, WorkflowService,
};
...
async fn update_with_local_acts() {
    ...
    let update = async {
        ...
        let updates = (1..=5).map(|_| handle.update(UpdateWithLocalActsWf::do_update, ()));
        for res in join_all(updates).await {
            assert!(res.unwrap() == "hi");
        }
        handle
            .signal(UpdateWithLocalActsWf::done_signal, ())
            .await
            .unwrap();
    };
    let run = async {
...
async fn update_rejection_sdk() {
    ...
    let handle = worker
        ...
        .unwrap();
    let update = async {
        let res = handle.update(UpdateRejectionSdkWf::do_update, ()).await;
        assert!(res.is_err());
    };
    let run = async {
        worker.run_until_done().await.unwrap();
...
async fn update_fail_sdk() {
    ...
    let handle = worker
        ...
        .unwrap();
    let update = async {
        let res = handle.update(UpdateFailSdkWf::do_update, ()).await;
        assert!(res.is_err());
    };
    let run = async {
        worker.run_until_done().await.unwrap();
...
async fn update_timer_sequence() {
    ...
    let handle = worker
        ...
        .unwrap();
    let update = async {
        let res = handle.update(UpdateTimerSequenceWf::do_update, ()).await;
...
async fn task_failure_during_validation() {
    ...
    let update = async {
        let res = handle
            .update(TaskFailureDuringValidationWf::do_update, ())
            .await;
        assert!(res.unwrap() == "done");
    };
    let run = async {
...
async fn task_failure_after_update() {
    ...
    let handle = worker
        ...
        .unwrap();
    let update = async {
        let res = handle.update(TaskFailureAfterUpdateWf::do_update, ()).await;
        assert!(res.unwrap() == "done");
    };
    let run = async {
        worker.run_until_done().await.unwrap();
...
async fn worker_restarted_in_middle_of_update() {
    ...
    let update = async {
        let res = handle
            .update(WorkerRestartedInMiddleOfUpdateWf::do_update, ())
            .await;
        assert!(res.is_ok());
        handle
            .signal(WorkerRestartedInMiddleOfUpdateWf::done_signal, ())
            .await
            .unwrap();
    };
...
async fn update_after_empty_wft() {
    ...
    let update = async {
        handle
            .signal(UpdateAfterEmptyWftWf::signal_handler, ())
            .await
            .unwrap();
        tokio::time::sleep(Duration::from_millis(500)).await;
        handle
            .update(UpdateAfterEmptyWftWf::do_update, ())
            .await
            .unwrap();
    };
    let runner = async {
...
async fn update_lost_on_activity_mismatch() {
    ...
    let update = async {
        ...
        for _ in 1..=2 {
            handle
                .update(UpdateLostOnActivityMismatchWf::do_update, ())
                .await
                .unwrap();
        }
    };
