...
//
// Each "step" is (Option<flag_to_wait_for>, vec_of_flags_to_set).
...
/// A single step: optionally wait for a flag, then set flags.
...
async fn execute_steps(ctx: &mut WorkflowContext<ChainWf>, steps: Vec<Step>) {
    for (wait_flag, set_flags) in steps {
        if let Some(w) = wait_flag {
...
/// the requested number of updates and signals, and asserts everything
/// completes within 5 seconds.
async fn run_convergence_test(name: &str, script: TestScript) {
...
#[tokio::test]
async fn convergence_update_workflow_update() {
...
#[tokio::test]
async fn convergence_update_cross_unblock() {
    run_convergence_test(
        "conv_upd_cross",
        TestScript {
...
#[tokio::test]
async fn convergence_signal_update_workflow() {
    run_convergence_test(
        "conv_sig_upd_wf",
        TestScript {
...
/// signal[0] only sees flag 1 after the convergence loop re-polls.
#[tokio::test]
async fn convergence_update_unblocks_signal() {
    run_convergence_test(
        "conv_upd_sig",
        TestScript {
            run_steps: vec![wait(2)],
...
#[tokio::test]
async fn convergence_full_chain() {
    run_convergence_test(
        "conv_full_chain",
        TestScript {
