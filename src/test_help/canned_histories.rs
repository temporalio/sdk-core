...
/// EVENT_TYPE_MARKER_RECORDED (Only present if param is not none)
/// EVENT_TYPE_TIMER_STARTED (id: "had_change" if marker present "no_change" if not)
/// EVENT_TYPE_TIMER_FIRED
/// EVENT_TYPE_WORKFLOW_TASK_SCHEDULED
/// EVENT_TYPE_WORKFLOW_TASK_STARTED
/// EVENT_TYPE_WORKFLOW_TASK_COMPLETED
/// EVENT_TYPE_WORKFLOW_EXECUTION_COMPLETED
pub fn has_change_different_timers(
    ...
    if let Some(id) = change_id {
        t.add_has_change_marker(id, deprecated);
    }
    let timer_id = if change_id.is_some() {
        "had_change"
    } else {
        "no_change"
    };
